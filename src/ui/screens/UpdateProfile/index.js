...
const UpdateProfile = ({ navigation }) => {
    ...
    const [birthday, setBirthday] = useState(profile.birthday == null ? '' : profile.birthday);
    const [email, setEmail] = useState(profile.email);
    const [phone, setPhone] = useState(profile.phone);
    const [userName, setUserName] = useState(user.object.username);
    const [zipCode, setZipCode] = useState(0);
    const [loading, setLoading] = useState(false);
    const [address, setAddress] = useState(profile.address == null ? '' : profile.address);
            ...
            <ScrollView style={styles.scrollContainer}>
                ...
                <ScrollView style={styles.scrollView} horizontal={true} showsHorizontalScrollIndicator={false}>
                    <View style={styles.imageRow}>
                        {[...memberPhotos, ...eventPhoto].map((image, index) => (
            <Image key={index} source={{ uri: image.uri || image.photo }} style={styles.uploadImage} />
            ))}
